...
import { getReadEcuDtcListCallEvent } from "socket/socketEmitters";
import {
   clearAllDTCListRedux,
   clearDTCListRedux,
   removeAllDTCListRedux,
   scanDTCListRedux,
} from "redux/ecuDtcList.slice";
...
const Dtccheck = () => {
   ...
   const [clearEcuName, setClearEcuName] = useState([]);
   ...
   const stationName = "DTC Check";
   const stationId = "st09";
   ...
   const handlePopup = (ecu, i) => {
      ...
      // getDTC(ecu)
      //    .then((res) => {
      //       console.log("eachDtc", res);
      //       setEachDtc(res?.data?.data);
      //    })
   ...
   const ClearDtc = (ecuName) => {
      ...
      dispatch(clearAllDTCListRedux(ecuName));
   ...
   const Submit = () => {
      ...
      dispatch(UpdateStationStatus("Short Track Test"));
   ...
   console.log("dtcList", filterData, dtcCodeTable, ecuName, clearedDtc);
   return (
