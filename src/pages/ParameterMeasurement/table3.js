...
export const Table12 = (props) => {
   ...
   const SelectGraph3 = (event) => {
      ...
      let state3 = Mgr1[event.target.innerText];
      Mgr1[event.target.innerText] = !state3;
      console.log("state3", Mgr1[event.target.innerText]);
   };
   const ecuParameterTable = useSelector(
      (state) => state?.getParameterData?.ecuParameter
   );
   const parameterdata = ecuParameterTable;
   // const parameterdata = [];
   // console.log("parameterdata", parameterdata,ecuParameterTable);
   let name = [];
   const Clearall = () => {
      setApex([]);
      setApexaxis([]);
   ...
   const parameterbody = parameterdata?.map((ele, i) => {
      ...
      console.log("object", ele["getEcuParameterMaxValue"]);
      return (
         <>
            {ele["getEcuParameterMaxValue"] === undefined ||
            ele["getEcuParameterMaxValue"] === 0 ? (
                  <tr key={i}>
                     <td>{ele["ecuName"]}</td>
                  ...
                  <tr key={i}>
                     ...
                     <td>{ele["ecuParameter"]}</td>
                     <td>
                        {Current_Value} {ele["ecuParameterUnit"]}
                     </td>
                     <td>
                        {ele["ecuParameterMinValue"]} {ele["ecuParameterUnit"]} -{" "}
                        {ele["getEcuParameterMaxValue"]} {ele["ecuParameterUnit"]}
                     </td>
                     <td key={i}>
                        <input
                           type="Checkbox"
      ...
      const interval = setInterval(() => {
         ...
         for (let i33 in list) {
            ...
            for (let i34 in CData) {
               console.log("CData[i34][list[i33]]", CData[i34][list[i33]]);
               if (CData[i34][list[i33]] !== undefined) {
   ...
   return (
      <>
         <div className="Parameter-Table">
            ...
            {parameterdata?.length !== props.count && (
               <CommonPopup
         ...
         <div className="table-button">
            {/* <button disabled={!data1}>Stop Monitoring</button> */}
            <button disabled={!data1} onClick={Clearall}>
               Clear all
            </button>
